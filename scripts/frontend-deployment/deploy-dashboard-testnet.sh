...
# Change directory to the dashboard package
...
echo "deploying dashboard for base mainnet $BASE_SEPOLIA_DASHBOARD_ID"
# Deploy for Base mainnet
netlify deploy --dir=. --prod --auth=$NETLIFY_AUTH_TOKEN --no-build --site $BASE_SEPOLIA_DASHBOARD_ID
echo "done"