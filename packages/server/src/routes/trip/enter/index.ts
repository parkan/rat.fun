...
// Lock
import { withTripEntryLock } from "@modules/lock"
  ...
  fastify.post(
    ...
    async (request: FastifyRequest<{ Body: SignedRequest<EnterTripRequestBody> }>, reply) => {
      ...
      try {
        ...
        // Acquire lock for this player to prevent concurrent trip entries
        // Lock is held until the trip entry process completes or errors
        // Automatically expires after 30 seconds as a safety measure
        return await withTripEntryLock(playerId, async () => {
          // * * * * * * * * * * * * * * * * * *
          // Get onchain data
          // * * * * * * * * * * * * * * * * * *
          ...
          logger.log(
            `✓ Onchain data fetched (${Math.round(performance.now() - onchainDataStart)}ms)`
          )
          ...
          const backgroundActions = async () => {
            try {
              ...
              const writeToPrivateCMS = writeOutcomeToPrivateCMS(
                ...
                handleBackgroundError(error, "Trip Entry - Private CMS", {
                  outcomeId,
                  tripId,
                  ratId
                })
              ...
              const writeToPublicCMS = writeOutcomeToCMS(
                ...
                handleBackgroundError(error, "Trip Entry - Public CMS", {
                  outcomeId,
                  tripId,
                  ratId
                })
