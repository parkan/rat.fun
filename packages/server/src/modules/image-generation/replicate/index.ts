...
const PREPROMPT = "STYLE: Fisheye distortion. Photorealistic. NEVER include humans. Cursed atmosphere. Liminal space. CONTENT: Security camera view of a room with: "
...
export const generateImage = async (prompt: string, levelPrompt: string) => {
  const INPUT = {
    FLUX: {
      ...
      prompt: `${PREPROMPT} ${prompt}. On a floor with: ${levelPrompt}`,
      go_fast: true,
      guidance: 3.5,
      megapixels: "1",
      num_outputs: 1,
