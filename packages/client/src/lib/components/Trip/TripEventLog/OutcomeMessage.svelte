...
<a href="/{outcome.tripId}/flashback/{outcome._id}" class="chat-event {topic}">
  {#if topic == "trip__outcome"}
    ...
    sent <span class="rat-name">{outcome.ratName}</span> to trip #{outcome.tripIndex ?? "unknown"}.
  {:else if topic == "rat__death"}
    ...
    let <span class="rat-name">{outcome.ratName}</span> die from trip #{outcome.tripIndex ??
      "unknown"}
  {:else}
    <!-- FALLBACK -->
